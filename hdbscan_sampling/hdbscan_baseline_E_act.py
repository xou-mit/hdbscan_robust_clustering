# XO note on 01/19/2022: This is the script for running HDBSCAN clustering code on input values sampled from their 
...
# Expected input: python filename.py zmax_cut rel_err_cut [N_samp - ignored] min_samples min_cluster_size data_dir [plots_dir]
...
data_dir = sys.argv[6] # xx Make it argument input
...
#         ydata_samp[:,j] = np.random.normal(ydata[:,j], ydata_err[:,j])
    ...
    tree_df.to_pickle(data_dir + 'tree' + extratext + '_baseline' + '.pkl',compression=None)
    ...
    if algorithm == 'HDBSCAN':
        ...
        f.savefig(plots_dir + 'clustering' + extratext + '_baseline' + '_tree' + '.png')
    ...
    if n_noise_ == 0:
        plot_summary_v3(df_cut_vel,df_cut_orb,ind_list=mask_list,c_list=col_list,
                        ...
                        filename=plots_dir+'summary'+extratext+ '_baseline' +'_v3.png')
#         plot_summary_v3(df_cut_vel,df_cut_orb,ind_list=mask_list,c_list=col_list,
#                         title=extratext,legends=lab_list,list_mask=False,ind_add_cut=ind_add_cut,ds=False,noise=False,
#                         filename=plots_dir+'summary'+extratext+ '_baseline' +'_v3_no_noise.png')
    else:
        plot_summary_v3(df_cut_vel,df_cut_orb,ind_list=mask_list,c_list=col_list,
